...
void structures::LinkedList<T>::invert() {
    ...
    iter->next(nullptr);
    Node* previousNode = iter;
    iter = nextNode;
    while (iter->next() != nullptr) {
        Node* nextNode = iter->next();
        iter->next(previousNode);
        previousNode = iter;
        iter = nextNode;
    }
    iter->next(previousNode);
...
structures::LinkedList<T> structures::LinkedList<T>::clone() {
    ...
    Node* iter = head;
    while (iter != nullptr) {
        list_clone.push_back(iter->data());
        iter = iter->next();
    }
    return list_clone;
}
...
structures::LinkedList<T> structures::LinkedList<T>::slicing(int start,
    ...
    if (start < 1 || start >= static_cast<int>(size()) ||
        stop < 0 || stop >= static_cast<int>(size()) || step <= 0) {
        throw std::out_of_range("Invalid index or step!");
    }
    ...
    for (int i = start; i < stop; i += step) {
        list_slice.push_back(iter->data());
...
void structures::LinkedList<T>::append(structures::LinkedList<T> *list_add) {
    ...
    Node* iter_add = list_add->head;
    while (iter_add != nullptr) {
        iter->next(new Node(iter_add->data()));
        iter = iter->next();
...
structures::LinkedList< structures::LinkedList<T> * >
    ...
    for (std::size_t i = 0; i < size(); i++) {
        if (i % 2 == 0) {
            list_even->push_back(at(i));
        } else {
            list_odd->push_back(at(i));
        }
    }
